...
# Function to generate the daily data at 9AM every day or nearest time of the day if 9:00AM data is not available
def generate_daily_snapshot(conn):
	with conn.cursor() as cursor:
		...
		for i in delta_date:
			cursor.callproc('UpdateDailySnapshotNearest', (i,))
	conn.commit()
