...
class TestSearchVolumeService(unittest.TestCase):
    ...
    @patch('question4.connect_db')
    def test_hourly_request_with_daily_subscription(self, mock_db):
        mock_conn = MagicMock()
        mock_cursor = MagicMock()
        mock_db.return_value = mock_conn
        mock_conn.cursor.return_value = mock_cursor
        ...
        mock_cursor.fetchone.return_value = {'is_active': True}
        mock_cursor.fetchall.return_value = [{'keyword_id': 1, 'keyword_name': 'crypto', 'subscription_type': 'daily'}]
        ...
        self.assertEqual(data['status'], 'success')
        print(data['data'])
        self.assertNotIn('python', data['data'])  # No hourly data returned
